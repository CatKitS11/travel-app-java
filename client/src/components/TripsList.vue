    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6"> <!-- EDIT: ปรับเป็น Grid 2 คอลัมน์ -->
      <div v-for="trip in trips" :key="trip.id" 
        ...
        <div class="sm:w-[350px] shrink-0 p-3">
          <div class="relative h-[300px] sm:h-full rounded-2xl overflow-hidden group-hover:shadow-sm transition-all bg-muted"> <!-- เพิ่ม bg-muted รองรับตอนไม่มีรูป -->
        ...
        <div class="flex-1 p-4 flex flex-col justify-between min-w-0">
          ...
          <div class="flex items-center justify-between pt-3 mt-auto border-t border-border/50 overflow-hidden"> <!-- เพิ่ม overflow-hidden ที่ parent -->
            ...
            <a :href="trip.url" target="_blank" class="shrink-0 px-4 py-2 rounded-full bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors whitespace-nowrap ml-2 shadow-sm">
              View more
            </a>
          </div>
        </div>
